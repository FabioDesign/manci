...
$(document).on('click', '.devlinemod', function(e){
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      if(splitter[6] == 1){
        $('.display-n').removeAttr('checked');
        $('.display-o').attr('checked', 'checked');
      }else{
        $('.display-o').removeAttr('checked');
        $('.display-n').attr('checked', 'checked');
      }
      if(splitter[7] != '') $('.form-command').html(splitter[7]);
